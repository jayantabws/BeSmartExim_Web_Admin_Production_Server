...
const DownloadTracker = () => {
  ...
  const handleSubmit = (values) => {
    let url = '';
    if (values.userId) {
      url = `search-management/search/listAll?userId=${values.userId}&isDownloaded=Y`;
    } else {
      url = `search-management/search/listAll?isDownloaded=Y`;
    }
    Axios({
      method: 'GET',
      url: url,
                    ...
                    <div className="form-group">
                      ...
                      >
                        ...
                        {userList && userList.length > 0
                          ? userList.map((title, qIndex) => (
                              <option value={title.id}>{title.firstname + '  ' + title.lastname + ' ( ' + title.email + ' )'}</option>
                            ))
                          : null}
